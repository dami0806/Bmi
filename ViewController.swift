...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
      ...
      setupUI()
        ...
        //계산하기 버튼 클릭시 1) 다음화면으로 이동
        
        //계산하기 버튼 클릭시 2) 다음화면에 정보전달

    }
    func setupUI(){
        textLabel.text = "키와 몸무게를 입력하세요"
    ...
    @IBAction func calulationBtnTapped(_ sender: UIButton) {
    ...
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        if(heightTextField.text == "" || weightTextField.text == ""){
            textLabel.text = "키와 몸무게를 모두 입력해주세요!"
...
//숫자이거나 지울때만 텍스트 입력 허용
// 키보드 엔터키가 눌렸을때 (키 ->몸무게 -> 키보드 내리기)
